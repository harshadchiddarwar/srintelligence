...
export async function classifyIntent(
  ...
  const systemPrompt = [
    ...
    '  CLUSTER           — segmentation or clustering analysis',
...
export async function decomposeIntoPipeline(params: {
  ...
  const systemPrompt = [
    ...
    '',
    'Rules:',
    '  1. The FIRST step MUST always be an ANALYST step to retrieve base data.',
    '  2. Use only valid AgentIntent values: ANALYST, FORECAST_PROPHET, FORECAST_SARIMA,',
    '     FORECAST_HW, FORECAST_XGB, FORECAST_COMPARE, FORECAST_AUTO, MTREE, CLUSTER.',
    '  3. dependsOn must reference stepIds of preceding steps only.',
    '  4. Respond with valid JSON only — no markdown fences, no explanation.',
  ].join('\n');
